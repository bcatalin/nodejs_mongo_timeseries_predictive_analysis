	...
	function getRecentSlope() {
		...
		if (data.recentTemp.length > 2) {
			for (var i = 0; i < data.recentTemp.length - 1; i++) {
				...
				var baseTemp  = data.recentTemp[i].tstamp_gmt.toISOString().replace(/T/, ' ').replace(/\..+/, '').trim();
				timeElementsBase = baseTemp.split(/[\s-:.]+/);
				incrementTemp = data.recentTemp[i + 1].tstamp_gmt.toISOString().replace(/T/, ' ').replace(/\..+/, '').trim();
				timeElementsIncrement = incrementTemp.split(/[\s-:.]+/);
				console.log("BASE: " + timeElementsBase);
				...
				dateIncrement = new Date(timeElementsIncrement[0],timeElementsIncrement[1], 
						timeElementsIncrement[2],timeElementsIncrement[3],
						timeElementsIncrement[4],timeElementsIncrement[5]);
